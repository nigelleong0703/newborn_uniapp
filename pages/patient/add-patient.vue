            ...
            <view class="form-content">
                <u--form labelPosition="left" :model="form1" ref="form1">
                    ...
                    <u-form-item label="性别" label-width="120" prop="gender" ref="item2">
                        <u-radio-group v-model="form1.gender">
                            <u-radio :customStyle="{ marginRight: '16px' }" v-for="(item, index) in genderlist"
                                :key="item.id" :label="item.name" :name="item.name" @change='sexSelect(item.id)'>
                            </u-radio>
                        </u-radio-group>
                    </u-form-item>
                    <u-form-item label="生日日期" label-width="120" prop="birthdate"
                        @click="time_picker = true; hideKeyboard()" ref="item3">
                    ...
                    <u-form-item label="监护人关系" label-width="120" prop="relation" ref="item6"
                        ...
                        <u--input v-model="form1.relation" disabled disabledColor="#ffffff" placeholder="请选择监护人关系"
                            border="none">
                        </u--input>
                    ...
                    <u-form-item label="入院日期" label-width="120" prop="inDate"
                        @click="inDate_picker = true; hideKeyboard()" ref="item8">
                        <u--input v-model="form1.inDate" placeholder="请输入入院日期" disabled disabledColor="#ffffff">
                        </u--input>
                    </u-form-item>
                    <u-form-item label="科室" label-width="120" prop="department" ref="item9"
                ...
                <u-datetime-picker ref="datetimePicker" :show="time_picker" mode="date" :minDate="mindate"
                    :maxDate="maxdate" closeOnClickOverlay @close="time_picker = false" @cancel="time_picker = false"
                    @confirm='time_select' @change='change' :formatter="formatter">
                </u-datetime-picker>
                <u-datetime-picker ref="inDatePicker" v-model="post.inDate" :show="inDate_picker" mode="datetime"
                    closeOnClickOverlay @close="inDate_picker = false" @cancel="inDate_picker = false"
                    @confirm='inDate_select' @change='change' :formatter="formatter">
                </u-datetime-picker>
            </view>
        </view>
...
export default {
    computed: {
        mindate: function () {
            return this.$common.deductYear(19)
        },
        maxdate: function () {
    ...
    onLoad(option) {
        ...
        let current_time = this.$common.loadSystemTime()
        this.post.inDate = current_time[0]
        this.form1.inDate = current_time[1]
    },
    onReady() {
    ...
    methods: {
        ...
        inDate_select(e) {
            var inDate = Math.round(e.value / 1000);
            this.form1.inDate = this.$common.dateTimeStr(inDate);
            this.post.inDate = inDate;
            this.inDate_picker = false
        }
    },
}
</script>
